        ...
        >
          <div className="flex flex-col gap-1">
            <h4 className="font-medium leading-none" id={labelId}>
              {filters.length > 0 ? "מסננים" : "לא הוחלו מסננים"}
            </h4>
            <p
              className={cn(
            ...
            >
              {filters.length > 0
                ? "שנה את המסננים כדי לסנן את השורות."
                : "הוסף מסננים כדי לסנן את השורות."}
            </p>
          </div>
          {filters.length > 0 ? (
    ...
    <SortableItem asChild value={filter.filterId}>
      ...
      >
        <div className="min-w-[72px] text-center">
          {index === 0 ? (
            <span className="text-muted-foreground text-sm">היכן</span>
          ) : index === 1 ? (
            <Select
            ...
            >
              <SelectTrigger
                ...
                aria-label="בחר אופרטור חיבור"
                className="h-8 rounded lowercase [&[data-size]]:h-8"
              >
                <SelectValue
                  placeholder={joinOperator === "and" ? "וגם" : "או"}
                />
              </SelectTrigger>
              <SelectContent
...
function onFilterInputRender<TData>({
  ...
  if (filter.operator === "isEmpty" || filter.operator === "isNotEmpty") {
    return (
      <div
        aria-label={`מסנן ${columnMeta?.label} הוא ${filter.operator === "isEmpty" ? "ריק" : "לא ריק"}`}
        aria-live="polite"
    ...
    case "range": {
      ...
      return (
        <Input
          ...
          aria-label={`ערך מסנן ${columnMeta?.label}`}
          ...
          placeholder={columnMeta?.placeholder ?? "הזן ערך..."}
      ...
      return (
        ...
        >
          <SelectTrigger
            ...
            aria-label={`מסנן בוליאני ${columnMeta?.label}`}
      ...
      return (
        ...
        >
          <FacetedTrigger asChild>
            <Button
              aria-controls={inputListboxId}
              aria-label={`ערך${multiple ? "י" : ""} מסנן ${columnMeta?.label}`}
          ...
          >
            <FacetedInput
              aria-label={`חיפוש אפשרויות ${columnMeta?.label}`}
              placeholder={columnMeta?.placeholder ?? "חיפוש אפשרויות..."}
            />
            <FacetedList>
              <FacetedEmpty>לא נמצאו אפשרויות.</FacetedEmpty>
              <FacetedGroup>
      ...
      return (
        <Popover onOpenChange={setShowValueSelector} open={showValueSelector}>
          <PopoverTrigger asChild>
            <Button
              aria-controls={inputListboxId}
              aria-label={`מסנן תאריך ${columnMeta?.label}`}
              className={cn(
                "w-full justify-start rounded text-right font-normal",
          ...
          >
            {filter.operator === "isBetween" ? (
              <Calendar
                aria-label={`בחר טווח תאריכים ${columnMeta?.label}`}
                dir="rtl"
                initialFocus
                mode="range"
                onSelect={(date) => {
              ...
              <Calendar
                aria-label={`בחר תאריך ${columnMeta?.label}`}
                dir="rtl"
                initialFocus
                mode="single"
                onSelect={(date) => {
